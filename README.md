...
1. Download [go from the official website](https://go.dev/doc/install).
2. Follow the [Go install](https://go.dev/doc/install) installation steps.
3. Set the `GOROOT` env variable, the location where you have untar the go:

    Example: if you untar the go tar to /usr/local then run the below command
    ```bash
    export GOROOT=/usr/local/go
    ```

4. Set the `GOPATH` env variable:
...
any certificates generated by LinaroCA. It must be consistent across the
deployment.
...
### Running `setup-bootstrap.sh` causes `Can't load /home/user/.rnd into RNG`

Run the `$openssl rand -writerand .rnd` command to generate missing .rnd file
