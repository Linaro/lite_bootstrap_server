...
The first script removes all previous build artifacts, builds linaroca,
generates new HTTP and CA keys, generates HTTP and CA certificates, and
start the CA server on port 1443.

The second script generates a new user key, signs it with the CA using the
REST API, and displays the output certificate.

The steps included in these scripts are detailed below.
